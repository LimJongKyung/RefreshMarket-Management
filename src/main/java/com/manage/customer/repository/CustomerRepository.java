...
public interface CustomerRepository extends JpaRepository<Customer, String> {
    ...
    @Query(value = """
	    SELECT
	        CASE
	            WHEN SUBSTR(SPLIT_PART(m.REGISTRATIONNUMBER, '-', 2), 1, 1) IN ('1','3') THEN '남자'
	            WHEN SUBSTR(SPLIT_PART(m.REGISTRATIONNUMBER, '-', 2), 1, 1) IN ('2','4') THEN '여자'
	            ELSE '기타'
	        END AS gender,
	        EXTRACT(YEAR FROM CURRENT_DATE) -
	            CASE
	                WHEN SUBSTR(SPLIT_PART(m.REGISTRATIONNUMBER, '-', 2), 1, 1) IN ('1','2')
	                    THEN 1900 + CAST(SUBSTR(m.REGISTRATIONNUMBER,1,2) AS INTEGER)
	                WHEN SUBSTR(SPLIT_PART(m.REGISTRATIONNUMBER, '-', 2), 1, 1) IN ('3','4')
	                    THEN 2000 + CAST(SUBSTR(m.REGISTRATIONNUMBER,1,2) AS INTEGER)
	                ELSE 0
	            END AS age,
